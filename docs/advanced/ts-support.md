...
`uni-network` 使用 TypeScript 编写，你可以享受到完整的 TypeScript 支持。
...
这可以通过设置两个泛型类型来解决，两个泛型类型分别对应响应数据和发送数据的类型。
...
这需要设置三个泛型类型来解决，三个泛型类型分别对应响应数据、发送数据、响应的类型。
...
⚠️ 注意：如果你只想修改响应的类型，而不修改其它类型，你仍然需要书写三个泛型类型。这和 `axios` 的泛型类型设计不同，因为 `uni-app` 对数据类型有更严格的要求。
